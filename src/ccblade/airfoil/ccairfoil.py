...
class CCAirfoil:
    ...
    def af_flap_coords(
        ...
        if multi_run or MPI_run:
            ...
            CoordsFlnmAF = f"profilecoords_p{pid}.dat"
            saveFlnmAF = f"{AFName}_{df}_Airfoil_p{pid}.txt"
            saveFlnmPolar = f"Polar_p{pid}.txt"
            xfoilFlnm = f"xfoil_input_p{pid}.txt"
            NUL_fname = f"NUL_{pid}"
        else:
            CoordsFlnmAF = "profilecoords.dat"  # Temporary file name for coordinates...will be deleted at the end
            saveFlnmPolar = "Polar.txt"
            saveFlnmAF = f"{AFName}_{df}_Airfoil.txt"
